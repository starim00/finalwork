...
public class FrmProductDetailManager_Add extends JDialog implements ActionListener {
	...
	public FrmProductDetailManager_Add(JDialog f, String s, boolean b, Map<Integer, BeanProduct> ptMap,
			Map<Integer, BeanRaw> rMap) {
		super(f, s, b);
		...
		i = 1;
		while (itRt1.hasNext()) {
			strTypes1[i] = Integer.toString(itRt1.next().getRawID());
			i++;
		}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == this.btnCancel) {
			...
			Integer ptID = new Integer(this.cmbProduct.getSelectedItem().toString());
			BeanProduct bp = this.productMap_name.get(ptID);
			if (bp == null) {
				JOptionPane.showMessageDialog(null, "请选择产品", "错误", JOptionPane.ERROR_MESSAGE);
				return;
			}
			bpd.setProductID(bp.getProductID());
			Integer rtID = new Integer(this.cmbRaw.getSelectedItem().toString());
			BeanRaw br = this.rawMap_name.get(rtID);
			if (br == null) {
